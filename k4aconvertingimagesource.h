...
public:
    inline ImageConversionResult GetNextImage(K4AViewerImage *textureToUpdate, k4a::image *sourceImage)
    {
		...
		GLenum result;
		if (ImageFormat == K4A_IMAGE_FORMAT_COLOR_BGRA32 || ImageFormat == K4A_IMAGE_FORMAT_COLOR_YUY2)
		{
			cv::Mat c_mat;
			ColorImageToMat(m_textureBuffers.CurrentItem()->Bgra.handle(), c_mat);

			k4abt_frame_t bodyFrame = NULL;
			K4ABodyTrackerManager::Instance().PopBodyFrameResult(bodyFrame);
			if (bodyFrame != NULL) {
				Visualize2DBody(bodyFrame, c_mat, K4ABodyTrackerManager::Instance().GetCameraCalibration());
			}

			k4a_image_t haha;
			MatToColorImage(c_mat, haha, K4A_IMAGE_FORMAT_COLOR_BGRA32);
			result = textureToUpdate->UpdateTexture(k4a::image(haha).get_buffer());
		}
		else if(ImageFormat == K4A_IMAGE_FORMAT_DEPTH16 || ImageFormat == K4A_IMAGE_FORMAT_IR16)
		{
			cv::Mat d_mat;
			ColorImageToMat(m_textureBuffers.CurrentItem()->Bgra.handle(), d_mat);
			
			k4abt_frame_t bodyFrame = NULL;
			K4ABodyTrackerManager::Instance().PopBodyFrameResult(bodyFrame);
			if (bodyFrame != NULL) {
				Visualize3DBody(bodyFrame, d_mat, K4ABodyTrackerManager::Instance().GetCameraCalibration());
			}

			k4a_image_t haha;
			MatToColorImage(d_mat, haha, K4A_IMAGE_FORMAT_COLOR_BGRA32);
			result = textureToUpdate->UpdateTexture(k4a::image(haha).get_buffer());
		}
		else
		{
			result = textureToUpdate->UpdateTexture(m_textureBuffers.CurrentItem()->Bgra.get_buffer());
		}
...
protected:
    void NotifyDataImpl(const k4a::capture &data)
    {
        ...
        if (image != nullptr)
        {
			//push into body tracker
			K4ABodyTrackerManager::Instance().EnqueueCaptureResult(data);
    ...
    static void WorkerThread(K4AConvertingImageSourceImpl *fs)
    {
        while (!fs->m_workerThreadShouldExit)
        {
            ...
            if (!fs->m_inputImageBuffer.Empty())
            {
				...
				//For converting depth image to RGB image, see 'k4adepthimageconverterbase'
